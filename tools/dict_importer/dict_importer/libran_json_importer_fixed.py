"""Fixed importer for the Librán dictionary JSON format."""
...
class LibranJSONImporterFixed:
    """Fixed importer for the specialized Librán dictionary JSON format."""
    ...
    def _process_entry(self, entry: Dict[str, Any], variant: str, cluster: str) -> None:
        ...
        if not english or not libran or libran == '—' or libran.startswith('—'):
        ...
        # Clean up the English word (lowercase for consistency)
        english_clean = english.lower().strip()
        
        # Clean up the Librán word - FIXED VERSION
        libran_clean = self._clean_libran_word_fixed(libran)
        ...
        if variant == 'ancient':
            self.ancient_entries[english_clean] = libran_clean
            self.stats['total_ancient'] += 1
        else:
            self.modern_entries[english_clean] = libran_clean
            self.stats['total_modern'] += 1
    
    def _clean_libran_word_fixed(self, word: str) -> str:
        """Clean up a Librán word - FIXED VERSION."""
        if not word or word == '—' or word.startswith('—'):
            return ""
        
        # Just strip whitespace and return the word as-is
        # The original cleaning was too aggressive and removed actual Librán words
        word = word.strip()
        
        # Only remove obvious non-Librán content like parenthetical English
        import re
        
        # Remove parenthetical English explanations (but keep Librán content)
        # This pattern looks for (English words) at the end
        word = re.sub(r'\([^)]*\)$', '', word)
        
        # Remove trailing English words that are clearly explanations
        # This is much more conservative than the original
        word = re.sub(r'\s+(Core|Sacred|Fate|Fire|flame|Concealment|Foundation|Wrath|Heart|force|Prophecy|Jealousy|Trust|belief|Dread|Blessing|Celebration|Affection|bond|Compassion|Grief|Prophetic|sight|Lineal|spirit|Revered|dead|Forebears|Bow|warrior|Singer|history|Kinship|Innocent|Bloodline|Collective|Old|age|wisdom|Opponent|Foe|Family|group|Weavers|destiny|kin|term|Tribal|patriarch|Champion|Brave|person|Our)$', '', word, flags=re.IGNORECASE)
        
        # Remove trailing Latin source words
        word = re.sub(r'\s+(statera|fatum|flamma|memoria|petra|ira|cor|vis|bucurie|milă|sorg|manes|sagittarius|bardus|frater|infans|antecessor|ætt|gens|aldri|hostis|nornir|pater|heros|rokon)$', '', word, flags=re.IGNORECASE)
        
        # Clean up any remaining whitespace
        word = word.strip()
        
        # Remove very short words (but be more lenient)
        if len(word) < 1:
            return ""
    ...
    def _save_report(self, output_path: Path) -> None:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("# Librán Dictionary Import Report (FIXED)\n\n")
...
def import_libran_dictionary_fixed(json_path: str, output_dir: str) -> DictionaryBuild:
    """Import the Librán dictionary from JSON and save to output directory - FIXED VERSION."""
    importer = LibranJSONImporterFixed()
